...
function actualizarMapaNacional() {
    ...
    // Actualizar gráfica de barras apiladas debajo del mapa
    crearGraficaBarrasMapaAcumulado();
}
...
function actualizarTablaDatos() {
    ...
    // Ordenar estados por acumulado descendente (de mayor a menor)
    const estadosOrdenados = [...datosNacionales].sort((a, b) => {
        const acumuladoA = datosAcumulados.find(e => e.estado === a.estado)?.valores[indiceFechaActual] || 0;
        const acumuladoB = datosAcumulados.find(e => e.estado === b.estado)?.valores[indiceFechaActual] || 0;
        return acumuladoB - acumuladoA; // Descendente: mayor primero
    });
...
// ===== GRÁFICA DE BARRAS PARA EL MAPA (ACUMULADO ENERO 2026) =====

let graficaBarrasMapaAcumuladoInstance = null;

async function crearGraficaBarrasMapaAcumulado() {
    const canvas = document.getElementById('graficaBarrasMapaAcumulado');
    if (!canvas) {
        console.error('❌ Canvas graficaBarrasMapaAcumulado no encontrado');
        return;
    }
    
    // Verificar que tengamos datos acumulados
    if (!datosAcumulados || datosAcumulados.length === 0) {
        console.error('❌ No hay datos acumulados disponibles');
        return;
    }
    ...
    // Destruir gráfica anterior si existe
    if (graficaBarrasMapaAcumuladoInstance) {
        graficaBarrasMapaAcumuladoInstance.destroy();
    }
    
    // Obtener la fecha actual seleccionada
    const fechaActual = fechasDisponibles[indiceFechaActual];
    if (!fechaActual) {
        console.error('❌ No hay fecha actual seleccionada');
        return;
    }
    
    // Obtener TOP 12 estados por acumulado hasta la fecha actual
    const estadosConAcumulado = datosAcumulados.map(e => ({
        estado: e.estado,
        acumuladoTotal: e.valores[indiceFechaActual] || 0,
        valores: e.valores
    })).sort((a, b) => b.acumuladoTotal - a.acumuladoTotal);
    
    const top12Estados = estadosConAcumulado.slice(0, 12);
    const entidades = top12Estados.map(e => e.estado);
    
    // Color para Enero (azul cielo de la paleta invertida)
    const colorEnero = '#42a5f5';
    
    // Crear dataset solo para Enero (acumulado hasta la fecha seleccionada)
    const datosEnero = top12Estados.map(estado => estado.acumuladoTotal);
    
    const datasets = [{
        label: 'Enero',
        data: datosEnero,
        backgroundColor: colorEnero,
        borderColor: colorEnero,
        borderWidth: 1
    }];
    
    graficaBarrasMapaAcumuladoInstance = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: entidades,
            datasets: datasets
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: 'Homicidios Dolosos 2026 - Acumulado',
                    font: { size: 16, weight: 'bold' }
                },
                legend: {
                    display: false  // Leyenda oculta
                },
                datalabels: {
                    display: true,
            ...
            scales: {
                ...
                y: {
                    stacked: false,
                    beginAtZero: true,
                    display: false // Ocultar eje Y según preferencia del usuario
                }
            }
        }
    });
    
    console.log('✓ Gráfica de barras (Enero) creada para el mapa');
}
